defmodule Razor.Generator do
  ...
  def generate(name, dir, prototype_repo \\ @default_repo) do
    ...
    with :ok <- check_target(dir),
         ...
         :ok <- Presenter.print_next_steps(dir) do
      Logger.info("Done.")
    else
      {:error, message} ->
  ...
  def rename_app(dir, name) do
    ...
    File.rename("#{dir}/lib/#{snake_proto_name}", "#{dir}/lib/#{snake_new_name}")
    File.rename("#{dir}/lib/#{snake_proto_name}_web", "#{dir}/lib/#{snake_new_name}_web")
    File.rename("#{dir}/test/#{snake_proto_name}_web", "#{dir}/test/#{snake_new_name}_web")
  ...
  defp check_response(%{status_code: status_code}),
    do: {:error, "Unhandled status code #{status_code}"}
